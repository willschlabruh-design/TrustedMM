...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // Prevent banning the platform middleman
  const target = await prisma.user.findUnique({ where: { id: userId } });
  if (!target) return res.status(404).json({ error: 'User not found' });
  if (target.role === 'ADMIN') return res.status(400).json({ error: 'Cannot ban platform admin' });

  const updated = await prisma.user.update({ where: { id: userId }, data: { role: 'BANNED' } });

  await prisma.notification.create({ data: { userId: updated.id, type: 'banned', payload: JSON.stringify({ reason: 'Banned by platform admin' }) } });
